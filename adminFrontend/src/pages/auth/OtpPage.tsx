					...
					<div className="relative flex h-full flex-col">
						...
						<div className="flex flex-1 items-center">
							<div className="max-w-xl py-12 lg:py-0">
								<p className="mb-6 text-4xl font-semibold leading-tight tracking-tight sm:text-5xl">
									{mode === 'forgot' ? 'Verify reset request' : 'Verify email registration'}
								</p>
								<p className="max-w-lg text-base leading-8 text-white/80 sm:text-lg">
									{mode === 'forgot'
										? 'A secure one-time passcode has been sent to verify your email. Please verify to reset your password.'
										: 'A secure one-time passcode has been sent to protect admin authorization. Please verify to finish account registration.'}
								</p>
							</div>
						</div>
				...
				<section className="flex items-center justify-center px-5 py-10 sm:px-8 lg:px-10 bg-white">
					<div className="w-full max-w-[480px]">
						<button
							...
							onClick={() => {
								if (mode === 'forgot') {
									...
									navigate('/auth/admin/signup', { state: { name, email, password } })
								}
							}}
						...
						>
							...
							<span>{mode === 'forgot' ? 'Back to forgot password' : 'Back to signup'}</span>
						</button>

						<div className="mb-8">
							<div className="text-sm font-semibold uppercase tracking-[0.22em] text-emerald-700">Email Verification</div>
							<h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900">Enter OTP Code</h1>
						...
						<form onSubmit={handleSubmit} className="space-y-5">
							<label className="block">
								<div className="mb-2 text-sm font-semibold text-slate-800">Verification Code</div>
								<div className="relative">
									<span className="pointer-events-none absolute inset-y-0 left-4 flex items-center text-slate-400">
										<svg viewBox="0 0 24 24" fill="none" className="h-5 w-5">
											...
											<path d="M19.5 10.5c0 6-7.5 10.5-7.5 10.5S4.5 16.5 4.5 10.5a7.5 7.5 0 0 1 15 0Z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
										</svg>
									</span>
									<input
										type="text"
										...
										className="h-14 w-full rounded-2xl border border-slate-200 bg-white pl-12 pr-4 text-sm text-slate-900 font-semibold tracking-[0.5em] placeholder:tracking-normal placeholder:font-normal shadow-[0_10px_24px_rgba(15,23,42,0.05)] outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10"
									/>
								</div>
							</label>
							...
							>
								{isLoading ? (
									...
									mode === 'forgot' ? 'Verify Code' : 'Verify & Create Account'
								)}
							</button>
							...
							<div className="text-center pt-2">
								<button
									type="button"
									disabled={isLoading || isResending || timer > 0}
									onClick={handleResend}
									className="text-sm font-semibold text-emerald-700 hover:text-emerald-800 disabled:opacity-50 disabled:cursor-not-allowed"
								>
									{isResending
										? 'Resending Code...'
										: timer > 0
