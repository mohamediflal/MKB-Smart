...
import { AlertTriangle, PackageX, Package, XCircle, History, Plus, Minus, Search } from "lucide-react";
import { Card, PageHeader, products, categories as globalCategories } from "../index";
...
export default function Inventory() {
  ...
  const categories = ["All categories", ...globalCategories.map((c) => c.name)];
  ...
  const handleAdjust = (id, amount) => {
    const updated = stockItems.map(p => {
      if (p.id === id) {
        ...
        const updatedItem = { ...p, stock: newStock };
  ...
  const getCategoryColor = (cat) => {
    const colors = {
      ...
      Staples: "bg-amber-50 text-amber-600",
        ...
        <Card className="rounded-[24px]">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <div className="text-3xl font-bold text-slate-900">{totalStockItems}</div>
            </div>
        ...
        <Card className="rounded-[24px]">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <div className="text-3xl font-bold text-slate-900">{inStock.length}</div>
            </div>
            <div className="h-12 w-12 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600">
              <Package size={24} />
            </div>
        ...
        <Card className="rounded-[24px]">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <div className="text-3xl font-bold text-slate-900">{lowStock.length}</div>
            </div>
        ...
        <Card className="rounded-[24px]">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <div className="text-3xl font-bold text-slate-900">{outOfStock.length}</div>
            </div>
                ...
                <tr key={p.id} className="bg-white hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`grid h-10 w-10 place-items-center rounded-full text-lg ${getCategoryColor(p.category)}`}>
                        {p.emoji}
                      </div>
