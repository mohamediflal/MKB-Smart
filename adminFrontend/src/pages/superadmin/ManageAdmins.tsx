...
import { Plus, MoreVertical, Pencil, Trash2, X, ShieldAlert, CheckCircle2, Eye, EyeOff } from "lucide-react";
...
export default function ManageAdmins() {
  ...
  const handleAddAdmin = async (e) => {
    ...
    if (password.length <= 8) {
      triggerToast("Password must be more than 8 characters.");
      return;
    }
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 pl-3.5 pr-10 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500 text-slate-800 dark:text-slate-200"
                    required
                  />
                  <button
                    type="button"
                ...
                {password && password.length <= 8 && (
                  <p className="mt-1 text-xs text-rose-500">Password must be more than 8 characters.</p>
                )}
              </div>
              <div>
